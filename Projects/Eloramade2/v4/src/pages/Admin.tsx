...
const handleViewScreenshot = async (order: any) => {
  if (!order.payment_screenshot_url) {
  ...
  try {
    // Get the path inside the bucket (remove domain parts)
    const path = order.payment_screenshot_url.split('/').slice(-2).join('/');

    // Only admin can generate signed URL
    const { data: signedUrlData, error } = await supabase.storage
      .from('payment-screenshots')
      .createSignedUrl(path, 60); // URL valid for 60 seconds

    if (error || !signedUrlData) throw error || new Error('Failed to generate URL');

    window.open(signedUrlData.signedUrl, '_blank');
  } catch (err: any) {
    toast.error(err.message || 'Cannot view screenshot');
  }
};
